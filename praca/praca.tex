...
\newline 
%Todo: monitorowac pozycje tej tabelki
...
Implementacja opiera się na wykorzystaniu prostych podstawowych funkcji z biblioteki libUsb. Rozwiązania zostały obudowane w odpowiednie klasy wraz z zastosowaniem polimorfizmu, aby uzyskać możliwość łatwego dodawania nowych udoskonaleń. Przed rozpoczęciem projektowania klas należało zaznajomić się ze schematem ideowym płytki LandTiger (Rys. \ref{fig:usbSchema}).
\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{./img/usbSchema}
\caption{Schemat ideowy \cite{landtigerDesc}} 
\label{fig:usbSchema}
\end{figure}
\noindent W projekcie wykorzystywano jedno z przedstawionych na rysunku złączy. Był nim USB\_Device, a co za tym idzie wszystkie zworki musiały zostać odpowiednio przestawione aby umożliwić jego działanie (JP9, JP10, JP11). Najistotniejsze jest to, iż D+ oraz D- przepuszczane są przez filtry dolnoprzepustowe. Istotne jest to ponieważ właśnie tymi interfejsami transmitowane są dane.
\begin{figure}[H]
\centering
...
\begin{lstlisting}[caption={Definicja przestrzeni nazw oraz odpowiednich struktur do poprawnej obslugi wątków},label={lst:ThreadHelper}]
...
\end{tabular}
\captionof{table}{Lista systemów operacyjnych na których wykonywano testy}
%source: 
